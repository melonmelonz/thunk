# kern Phase 1 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A working offline `kern` binary that teaches Module 1 (The Kernel) end-to-end from one
content source, and boots a test pattern on a simulated panel — demoable at the Aug 2026 Next
Chapter demo.

**Architecture:** A Cargo workspace of small, focused crates. `kern-core` holds pure, tested domain
logic (content model, check evaluation, progress). `kern-content` embeds the Module 1 curriculum at
compile time via `rust-embed`. `kern-sim` is a deterministic pure-Rust model of an SPI bus + panel
framebuffer. `kern-cli` (native, crossterm) and later `kern-tui` (ratatui) render the same content.
...
kern/
  ...
  kern-cli/
    Cargo.toml                    # bin name = "kern"
    src/main.rs                   # clap entrypoint
    src/cmd/{read.rs,check.rs,progress.rs,sim.rs,mod.rs}
```

`kern-tui` is added at Task 9. Split by responsibility: domain (`core`), assets (`content`),
...
- [ ] **Step 1:** `cd ~/dev/kern && git init`.
- [ ] **Step 2:** Write workspace `Cargo.toml` with members `kern-core, kern-content, kern-sim, kern-cli` and a `[workspace.dependencies]` block (serde, ron, rust-embed, clap, anyhow, thiserror, ratatui, crossterm). Release profile: `lto="thin"`, `codegen-units=1`, `strip=true`.
- [ ] **Step 3:** Write `rust-toolchain.toml` pinning `channel = "1.88"` with rustfmt+clippy.
- [ ] **Step 4:** `cargo new --lib kern-core && cargo new --lib kern-content && cargo new --lib kern-sim && cargo new kern-cli` (fix bin name to `kern` in `kern-cli/Cargo.toml`).
- [ ] **Step 5:** Add MIT/Apache license files and `.gitignore` (`/target`).
- [ ] **Step 6:** Run `cargo build` — expect clean empty build.
- [ ] **Step 7:** Commit: `git add -A && git commit -m "chore: scaffold kern workspace"`.

## Task 1: kern-core content model

**Files:** Create `kern-core/src/content.rs`; modify `kern-core/src/lib.rs`.

- [ ] **Step 1: Write the failing test** in `kern-core/src/content.rs`:
...
- [ ] **Step 2: Run to verify it fails.** Run: `cargo test -p kern-core`. Expected: FAIL (types undefined).
- [ ] **Step 3: Implement** in `kern-core/src/content.rs`:
...
- [ ] **Step 4:** Add `pub mod content; pub use content::*;` to `kern-core/src/lib.rs`; add `serde` dep in `kern-core/Cargo.toml` (`serde = { workspace = true }`).
- [ ] **Step 5: Run to verify pass.** Run: `cargo test -p kern-core`. Expected: PASS.
- [ ] **Step 6: Commit:** `feat(core): module/lesson content model`.

## Task 2: kern-core check evaluation

**Files:** Create `kern-core/src/check.rs`; modify `lib.rs`.
...
- [ ] **Step 2:** Run `cargo test -p kern-core` — expect FAIL.
- [ ] **Step 3: Implement** `kern-core/src/check.rs`:
...
- [ ] **Step 4:** Export in `lib.rs`. **Step 5:** `cargo test -p kern-core` PASS. **Step 6:** Commit `feat(core): check evaluation`.

## Task 3: kern-core progress / mastery

**Files:** Create `kern-core/src/progress.rs`.
...
- [ ] **Step 4:** export; add `LessonId`/`CheckId` derive `Ord` (needed for BTreeSet — add `PartialOrd, Ord` to their derives in content.rs/check.rs). **Step 5:** `cargo test -p kern-core` PASS. **Step 6:** Commit `feat(core): progress + mastery`.

## Task 4: kern-content — embed and load Module 1

**Files:** Create `kern-content/modules/m1-kernel/{module.ron,01-programs-and-os.md,checks.ron}`, `kern-content/src/lib.rs`.
...
- [ ] **Step 4: Failing test** in `kern-content/src/lib.rs`:
...
- [ ] **Step 5:** run, expect FAIL. **Step 6: Implement** loader with `rust-embed` (folder = `modules/`), parse `module.ron` + read each lesson `.md` into `Lesson`, parse `checks.ron`. Add `kern-core`, `serde`, `ron`, `rust-embed` deps. **Step 7:** `cargo test -p kern-content` PASS. **Step 8:** Commit `feat(content): embed + load Module 1`.

## Task 5: kern-sim — SPI bus model + trace

**Files:** Create `kern-sim/src/spi.rs`, `kern-sim/src/lib.rs`.
...
## Task 6: kern-sim — panel framebuffer

**Files:** Create `kern-sim/src/panel.rs`.
...
## Task 7: kern-sim — boot a test pattern

**Files:** Create `kern-sim/src/boot.rs`.
...
## Task 8: kern-cli — native entrypoint + subcommands

**Files:** Create `kern-cli/src/main.rs`, `kern-cli/src/cmd/{mod.rs,read.rs,check.rs,progress.rs,sim.rs}`.
...
- [ ] **Step 3: Integration test** (`kern-cli/tests/cli.rs`) using `assert_cmd`: `kern read 01-programs-and-os` exits 0 and stdout contains "kernel".
- [ ] **Step 4:** run `cargo test -p kern-cli` PASS. **Step 5:** `cargo run -p kern-cli -- sim` shows color bars as ASCII. **Step 6:** Commit `feat(cli): read/check/progress/sim subcommands`.

## Task 9: kern-tui — terminal classroom

**Files:** Create `kern-tui/` crate; scenes for lesson reader, check, progress, and a panel view rendering the sim framebuffer as colored blocks.

- [ ] **Step 1:** Add `kern-tui` to workspace; ratatui + crossterm (native), state driven by `kern-core` + `kern-content` + `kern-sim`.
...
- [ ] **Step 5:** Unit-test the scene *state* transitions (pure), not the draw calls. **Step 6:** Wire `kern` default subcommand to launch the TUI. **Step 7:** Commit `feat(tui): reader + check + panel scenes`.

## Task 10: Author Module 1 content in full

**Files:** `kern-content/modules/m1-kernel/*.md` + `checks.ron`.

- [ ] **Step 1:** Author lessons 01–05: programs vs OS; the kernel and privilege; syscalls; files & devices; mmap. Engineering voice, ~40–70 lines each, "key terms" footer. (Draw on the proven arc from `~/dev/peek/peek-content/chapters` but rewritten fresh for kern; do not copy PEEK's game framing.)
